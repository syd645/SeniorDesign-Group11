...
def load_data(user_id):
	...
	for class_id in range(NUM_OF_CLASS):
		...
		read_path = './HAR_LS/large_scale_HARBox/' +  str(user_id) + '/' + str(class_set[class_id]) + '_train' + '.txt'
...
for current_user_id in range(NUM_OF_TOTAL_USERS):
	if(current_user_id == 0):
		...
		y_coll = y_coll.reshape(-1, 1)
		allData = np.hstack((x_coll,y_coll))
	else:
		print("USER:", current_user_id)
		x_coll, y_coll, dimension, num_of_class = load_data(current_user_id)
		print(count_analysis(y_coll))
		count_user_data[current_user_id] = y_coll.shape[0]
		y_coll = y_coll.reshape(-1, 1)
		temp = np.hstack((x_coll,y_coll))
		allData = np.vstack((allData,temp))

dataset = torch.from_numpy(allData)
torch.save(dataset, 'LS_HAR_data.pt')
